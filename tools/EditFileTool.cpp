    ...
    return QtConcurrent::run([this, input]() -> QString {
        ...
        for (const SearchReplaceBlock &block : blocks) {
            ...
            LOG_MESSAGE(QString("EditFileTool: Processing block %1 for %2:")
                           .arg(blockIndex + 1).arg(filePath));
            LOG_MESSAGE(QString("  searchContent length: %1 chars").arg(block.searchContent.length()));
            LOG_MESSAGE(QString("  replaceContent length: %1 chars").arg(block.replaceContent.length()));
            ...
            LOG_MESSAGE(QString("File edit created: %1 (ID: %2, Status: %3, Block: %4)")
                            .arg(filePath, editId, status).arg(blockIndex));
