    ...
    int lineHintEnd)
{
    ...
    if (!requestId.isEmpty() && !isFromHistory) {
        ...
        LOG_MESSAGE(
            QString("File edit tracked for request: %1 (requestId: %2)").arg(editId, requestId));
    }
    ...
    const QString &filePath, const QString &oldContent, const QString &newContent, QString *errorMsg)
{
    ...
    for (auto *editor : editors) {
        ...
        if (editorPath == filePath) {
            ...
            if (oldContent.isEmpty()) {
                if (auto *textEditor = qobject_cast<TextEditor::TextDocument *>(
                        editor->document())) {
            ...
            if (matchPos != -1) {
                ...
                QString matchedContent
                    = findBestMatch(currentContent, oldContent, 0.82, &similarity);
                if (!matchedContent.isEmpty()) {
                    ...
                    if (matchPos != -1) {
                        if (auto *textEditor = qobject_cast<TextEditor::TextDocument *>(
                            ...
                            cursor.setPosition(matchPos);
                            cursor.setPosition(
                                matchPos + matchedContent.length(), QTextCursor::KeepAnchor);
                            cursor.removeSelectedText();
                            cursor.insertText(newContent);
                            cursor.endEditBlock();
                            ...
                            LOG_MESSAGE(QString("Updated open editor (fuzzy match %1%%): %2")
                                            ...
                                            .arg(filePath));
                            setError(QString("Applied with fuzzy match (%1%% similarity)")
                                         .arg(qRound(similarity * 100)));
                            return true;
                        }
                    }
                ...
                LOG_MESSAGE(
                    QString("Old content not found in open editor (best similarity: %1%%): %2")
                        .arg(qRound(similarity * 100))
                        .arg(filePath));
                setError(QString(
                             "Content not found. Best match: %1%% (threshold: 82%%). "
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        LOG_MESSAGE(
            QString("Failed to open file for reading: %1 - %2").arg(filePath, file.errorString()));
        setError(msg);
        return false;
    }
    ...
    if (oldContent.isEmpty()) {
        ...
        int matchPos = currentContent.indexOf(oldContent);
        updatedContent = currentContent.left(matchPos) + newContent
        ...
        if (!matchedContent.isEmpty()) {
            ...
            LOG_MESSAGE(QString("Using fuzzy match (%1%%) for file update: %2 at position %3")
                            ...
                            .arg(matchPos));
            setError(
                QString("Applied with fuzzy match (%1%% similarity)").arg(qRound(similarity * 100)));
        } else {
            QString msg = QString(
                              "Content not found. Best match: %1%% (threshold: 82%%). "
                              "File may have changed.")
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate)) {
        ...
        LOG_MESSAGE(
            QString("Failed to open file for writing: %1 - %2").arg(filePath, file.errorString()));
        setError(msg);
        return false;
    }
...
QString ChangesManager::findBestMatch(
    const QString &fileContent,
    const QString &searchContent,
    double threshold,
    double *outSimilarity) const
{
    ...
    if (searchLen > MAX_SEARCH_LENGTH) {
        LOG_MESSAGE(
            QString("Search content too large (%1 chars), using line-based search").arg(searchLen));
            ...
            if (similarity >= threshold) {
                ...
                if (similarity >= 0.95) {
                    if (outSimilarity)
                        *outSimilarity = bestSimilarity;
                    LOG_MESSAGE(
                        QString("Found excellent match early (similarity: %1%%), stopping search")
                            .arg(qRound(similarity * 100)));
                    return bestMatch;
                }
            }
    ...
    if (!bestMatch.isEmpty()) {
        LOG_MESSAGE(
            QString("Fuzzy match found with similarity: %1%%").arg(qRound(bestSimilarity * 100)));
    } else {
        LOG_MESSAGE(QString("No match found above threshold. Best similarity: %1%%")
                        .arg(qRound(bestSimilarity * 100)));
    }
    ...
    int *outPosition) const
{
    ...
    if (!bestMatch.isEmpty() && bestSim >= 0.70) {
        LOG_MESSAGE(
            QString("Match found: Fuzzy match (%1%% similarity)").arg(qRound(bestSim * 100)));
        if (outSimilarity)
            *outSimilarity = bestSim;
        if (outMatchType) {
            if (bestSim >= 0.85) {
    ...
    LOG_MESSAGE(QString(
                    "Cannot proceed: similarity too low (%1%%). "
    ...
    int lineHintEnd)
{
    ...
    if (isAppendOperation) {
        if (searchContent.isEmpty()) {
            if (lineHintStart > 0 && lineHintEnd > 0) {
                // Insert after the hinted line instead of appending to end
                QStringList lines = currentContent.split('\n');
                int insertLine = lineHintStart - 1; // Convert to 0-based
                if (insertLine >= 0 && insertLine < lines.size()) {
                    QStringList newLines = replaceContent.split('\n');
                    // Insert after the line
                    lines = lines.mid(0, insertLine + 1) + newLines + lines.mid(insertLine + 1);
                    resultContent = lines.join('\n');
                    if (errorMsg)
                        *errorMsg
                            = QString("Successfully inserted after line %1").arg(lineHintStart);
                } else {
                    // Fallback to append
                    resultContent = currentContent + replaceContent;
                    if (errorMsg)
                        *errorMsg = "Successfully applied (appended to end)";
                }
            } else {
                resultContent = currentContent + replaceContent;
                if (errorMsg)
            ...
            if (currentContent.endsWith(searchContent)) {
                resultContent = currentContent.left(
                    currentContent.length() - searchContent.length());
                if (errorMsg)
        ...
        if (!matchedContent.isEmpty() && similarity < minThreshold) {
            QString msg = QString("Cannot %1: similarity too low (%2%%, threshold: %3%%). %4")
                              ...
                              .arg(qRound(minThreshold * 100))
                              .arg(
                                  isUndo ? "File may have been modified."
        ...
        if (!matchedContent.isEmpty()) {
            ...
            LOG_MESSAGE(
                QString("Failed to find content for fragment replacement: %1").arg(filePath));
            return false;
        }
    }
            ...
            if (auto *textEditor = qobject_cast<TextEditor::TextDocument *>(editor->document())) {
                ...
                try {
                    ...
                    if (!cursor.isNull()) {
                        ...
                        LOG_MESSAGE(QString("Applied fragment replacement to open editor: %1")
                                        .arg(filePath));
                        return true;
                    }
                } catch (...) {
                    ...
                    if (errorMsg)
                        *errorMsg = "Exception during document modification";
                    return false;
                }
            }
        }
...
bool ChangesManager::undoAllEditsForRequest(const QString &requestId, QString *errorMsg)
{
    ...
    LOG_MESSAGE(
        QString("Undoing %1 edits for request: %2").arg(reqEdits.editIds.size()).arg(requestId));
    ...
    for (int i = reqEdits.editIds.size() - 1; i >= 0; --i) {
        ...
        if (edit.status != Applied) {
            LOG_MESSAGE(QString("Edit %1 is not applied (status: %2), skipping")
                            .arg(editId)
                            .arg(edit.status));
            continue;
        }
        ...
        LOG_MESSAGE(
            QString("Undoing edit %1 using REVERSE fragment replacement (mass undo)").arg(editId));
...
bool ChangesManager::reapplyAllEditsForRequest(const QString &requestId, QString *errorMsg)
{
    ...
    LOG_MESSAGE(
        QString("Reapplying %1 edits for request: %2").arg(reqEdits.editIds.size()).arg(requestId));
    ...
    for (const QString &editId : reqEdits.editIds) {
        ...
        LOG_MESSAGE(
            QString("Reapplying edit %1 using fragment replacement (mass apply)").arg(editId));
...
bool ChangesManager::performFileEditWithDiff(
    const QString &filePath, const DiffInfo &diffInfo, bool reverse, QString *errorMsg)
{
    ...
    const QString &originalContent, const QString &modifiedContent, const QString &filePath)
{
    ...
    if (hunks.isEmpty() && originalContent != modifiedContent) {
        ...
        LOG_MESSAGE(
            QString("=== DiffInfo created successfully with %1 hunk(s) ===").arg(hunks.size()));
    }
    ...
    QString *debugInfo) const
{
    ...
    if (expectedIdx >= 0 && expectedIdx < fileLines.size()) {
        ...
        if (checkIdx < 0) {
            ...
            for (int i = 0; i < hunk.contextBefore.size(); ++i) {
                if (fileLines[checkIdx + i] != hunk.contextBefore[i]) {
                    exactMatch = false;
                    debug
                        += QString("    Context before mismatch at offset %1: expected '%2', got '%3'\n")
                               .arg(i)
                               .arg(hunk.contextBefore[i])
                               .arg(fileLines[checkIdx + i]);
                    break;
                }
            }
        }
        ...
        if (exactMatch) {
            for (int i = 0; i < hunk.removedLines.size(); ++i) {
                ...
                if (lineIdx >= fileLines.size() || fileLines[lineIdx] != hunk.removedLines[i]) {
                    ...
                    debug
                        += QString(
                               "    Removed line mismatch at offset %1: expected '%2', got '%3'\n")
                               .arg(i)
        ...
        if (exactMatch && !hunk.contextAfter.isEmpty()) {
            ...
            for (int i = 0; i < hunk.contextAfter.size(); ++i) {
                ...
                if (lineIdx >= fileLines.size() || fileLines[lineIdx] != hunk.contextAfter[i]) {
                    ...
                    debug
                        += QString(
                               "    Context after mismatch at offset %1: expected '%2', got '%3'\n")
                               .arg(i)
        ...
        if (exactMatch) {
            ...
            if (debugInfo)
                *debugInfo = "Exact match at expected location";
            return true;
        } else {
    ...
    int totalPossibleScore = hunk.contextBefore.size() + hunk.removedLines.size()
                             + hunk.contextAfter.size();
    double matchPercentage = totalPossibleScore > 0
                                 ? (double) bestMatchScore / totalPossibleScore * 100.0
                                 : 0.0;
    ...
    LOG_MESSAGE(
        QString("  ✗ Hunk location not found (best match: %1%%)").arg(matchPercentage, 0, 'f', 1));
...
bool ChangesManager::applyDiffToContent(
    QString &content, const DiffInfo &diffInfo, bool reverse, QString *errorMsg)
{
    ...
    if (diffInfo.useFallback) {
        ...
        if (matchPos != -1) {
            content = content.left(matchPos) + replaceContent
                      + content.mid(matchPos + searchContent.length());
            setError("Applied using fallback mode (direct replacement)");
            LOG_MESSAGE(
                QString("  ✓ Fallback: Direct replacement successful at position %1").arg(matchPos));
            return true;
        } else {
    ...
    LOG_MESSAGE(QString("  File has %1 lines, applying %2 hunk(s)")
                    .arg(fileLines.size())
                    .arg(diffInfo.hunks.size()));

    QList<DiffHunk> hunksToApply = diffInfo.hunks;

    std::sort(hunksToApply.begin(), hunksToApply.end(), [](const DiffHunk &a, const DiffHunk &b) {
    ...
    for (int hunkIdx = 0; hunkIdx < hunksToApply.size(); ++hunkIdx) {
        ...
        LOG_MESSAGE(
            QString("  --- Applying hunk %1/%2 ---").arg(hunkIdx + 1).arg(hunksToApply.size()));
        ...
        if (!findHunkLocation(fileLines, hunk, actualStartLine, &debugInfo)) {
            LOG_MESSAGE(
                QString("  ✗ Failed to locate hunk %1:\n%2").arg(hunkIdx + 1).arg(debugInfo));
            failedHunks++;
            continue;
        }
